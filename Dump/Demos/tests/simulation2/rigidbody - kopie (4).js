    ...
    constructor(canvas){
        ...
        class Object{
            ...
            resolveBoundaries(conatiner){
                ...
                if(this.pos[1]-this.r<conatiner.top) this.pos[1] = conatiner.left + this.r
                if(this.pos[1]+this.r>conatiner.bottom) this.pos[1] = conatiner.right - this.r
            }
            ...
            resolveCollisions(){
                for (var object of objects) {
                    ...
                    if(circle2!=circle1){
                        var xdist =circle2.pos[0]-this.pos[0], ydist =circle2.pos[1]-this.pos[1];
                        
                        var distance = Math.sqrt((xdist)**2+(ydist)**2);
                        var angle = Math.atan2(ydist, xdist);
                        var toMove = (circle2.r+this.r - distance)/2;
                        var xToMove = Math.cos(angle) * toMove, yToMove = Math.sin(angle) * toMove;
                        //canvas.drawText(Math.round(xToMove) +", "+ Math.round(yToMove)+": "+angle, this.pos[0], this.pos[1]-11*index)
                        if(distance<this.r+circle2.r) object.pos.add(new Vector(xToMove, yToMove)), this.pos.add(new Vector(-xToMove, -yToMove))//this.vel[0] -= xToMove, this.vel[1] -= yToMove, circle2.xv += xToMove, circle2.yv += yToMove;
                        if(distance<this.r+circle2.r) object.acc = new Vector(0, 0)
                    }
                }
            }
